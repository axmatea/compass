  ...
  plugins: [react(), {
    ...
    configureServer(server) {
      server.middlewares.use((req, _res, next) => {
        ...
        else if (req.url && /^\/presentation\/legacy\/?(?:\?|$)/.test(req.url)) req.url = "/story.html";
        else if (req.url && /^\/(app|demo\/(?:table|workspace|remaster(?:\/app)?)|presentation|present|story)\/?(?:\?|$)/.test(req.url)) req.url = "/index.html" + (req.url.includes("?") ? req.url.slice(req.url.indexOf("?")) : "");
        next();
      });
    },
