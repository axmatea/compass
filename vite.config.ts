  ...
  plugins: [react(), {
    ...
    configureServer(server) {
      server.middlewares.use((req, _res, next) => {
        ...
        else if (req.url && /^\/(app|demo(?:\/(?:workspace|remaster(?:\/app)?))?|presentation|present|story)\/?(?:\?|$)/.test(req.url)) req.url = "/index.html" + (req.url.includes("?") ? req.url.slice(req.url.indexOf("?")) : "");
        next();
      });
    },
