...
const server = createServer(async (req,res) => {
 ...
 try {
  ...
  if(['/present','/present/'].includes(pathname)){res.writeHead(301,{Location:'/presentation'}).end();return;} // the presentation lives at /presentation
  const route = ['/'].includes(pathname) ? '/index.html' : ['/presentation','/presentation/','/story','/story/'].includes(pathname) ? '/story.html' : ['/demo','/demo/'].includes(pathname) ? '/live.html' : ['/horizon','/horizon/'].includes(pathname) ? '/horizon.html' : pathname;
